...
# ╔═╡ 8ca110de-254c-4cac-ad85-0f0104856c31
begin
	# Define true Gaussian mixture model parameters:
	w = [0.5, 0.5]
	mu = [0, 5]
	mixturemodel = MixtureModel(Normal[
		Normal(mu[1], 1),
		Normal(mu[2], 1)
		],
		[w[1], w[2]]
	...
	# Draw & plot N data points:
	Random.seed!(4389) # explicit redef. of seed for reproducibility
	N = 10
	x = rand(mixturemodel, N)
	histogram(x, bins=10; legend=false, title="Synthetic data")
end

# ╔═╡ 9156dbf1-2a98-4206-937e-e9dbd221b271
# Grid-approx. Gaussian Mixture update, for toying purposes.
...
begin
	...
	# Scale probabilities for exponentiation during later sampling
	prob_model = prob_model .- maximum(prob_model) .+ 10
end

# ╔═╡ f237cf8d-1f8f-49af-a2dc-2c9043c4247c
...
begin
	...
	N_samples = 1000
	posterior_sample_indices = sample(eachindex(prob_model), Weights(vec(exp.(prob_model))), N_samples)

	posterior_samples = CartesianIndices(prob_model)[posterior_sample_indices]
	mu1_posterior_samples = mu_grid[getindex.(posterior_samples, 1)]
	mu2_posterior_samples = mu_grid[getindex.(posterior_samples, 2)]
	c1_posterior_samples = c_grid[getindex.(posterior_samples, 3)]

	ppd_data = []
	for i in range(1, N_samples)
		sampled_model = MixtureModel(Normal[
			Normal(mu1_posterior_samples[i], 1),
			Normal(mu2_posterior_samples[i], 1)
		],
		[c1_posterior_samples[i], 1-c1_posterior_samples[i]]
		)
		ppd_sample = rand(sampled_model, 1)
		append!(ppd_data, ppd_sample)
	end

	histogram(ppd_data, bins=100; title="Posterior predictive distribution")
